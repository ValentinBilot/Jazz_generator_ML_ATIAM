...
for filename in filenames:
	...
	midifile = rootname + "/" + filename + "/" + os.listdir(rootname+"/"+filename)[0]
	...
	for mychord in chords:
		if type(mychord) != chord.Chord:
			#do nothing
			print("doing nothing because it's type :", type(mychord))
			...
			while time_write < time :
				...
				if name[0:5] == "minor":
					...
					harmo = "maj"
				elif name[0:16] == "incomplete minor":
					harmo = "min"
				elif name[0:16] == "incomplete major":
					harmo = "maj"
				elif name == "incomplete dominant-seventh chord":
					harmo = "maj"
				elif name == "augmented major tetrachord":
					harmo = "maj"
				elif name[0:8] == "dominant":
					harmo = "maj"
				elif name == "perfect-fourth major tetrachord":
					harmo = "maj"
				else:
					harmo = "N"
				if harmo == "N":
					listToWrite.append(str(time_write)+" "+"N")
				else:
